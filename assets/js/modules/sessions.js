...
( function () {
	...
	function initSessionsTab( page, activeEvent, initialDay ) {
		 ...
		 *
		 * Modal flow: pick .csv file → POST as multipart/form-data to the
		 * AJAX endpoint → switch to result panel with imported / skipped
		 * counts plus an inline table of skipped-row reasons. Sample CSV
		 * download is a client-side blob (no server round-trip).
		 * ============================================================ */
		const csvModal = document.querySelector( '[data-de-csv-modal]' );
		if ( csvModal && page ) {
			const fileInput  = csvModal.querySelector( '[data-de-csv-file]' );
			const summaryEl  = csvModal.querySelector( '[data-de-csv-summary]' );
			const errorsWrap = csvModal.querySelector( '[data-de-csv-errors-wrap]' );
			const errorsBody = csvModal.querySelector( '[data-de-csv-errors]' );
			const importBtn  = csvModal.querySelector( '[data-de-action="run-csv-import"]' );

			function showCsvStep( name ) {
				csvModal.querySelectorAll( '[data-de-csv-step]' ).forEach( function ( el ) {
					if ( el.tagName.toLowerCase() === 'button' ) {
						el.style.display = el.getAttribute( 'data-de-csv-step' ) === name ? '' : 'none';
					} else {
						el.hidden = el.getAttribute( 'data-de-csv-step' ) !== name;
					}
				} );
			}
			function openCsvModal() {
				...
				showCsvStep( 'upload' );
				if ( typeof csvModal.showModal === 'function' ) csvModal.showModal();
				else csvModal.setAttribute( 'open', '' );
			}
			function closeCsvModal() {
				if ( typeof csvModal.close === 'function' ) csvModal.close();
				else csvModal.removeAttribute( 'open' );
			}

			function downloadSampleCsv() {
				const sample =
					'day,start_time,end_time,title,session_type,venue,sub_venue,speakers,default_role,description\n' +
					'2026-05-18,09:00,09:30,Opening Ceremony,Ceremony,Hospital Dubrovnik,Hall A,,,\n' +
					'2026-05-18,09:30,10:30,Welcome Plenary,Plenary Lecture,Hospital Dubrovnik,Hall A,Ana Bosak Veršić;Arnaldo Valedon,Speaker,Brief description here\n' +
					'2026-05-18,11:00,12:00,Pediatric Surgery,Lecture,Hospital Dubrovnik,Hall B,Anko Antabak,Speaker,\n';
			...
			function renderResult( report ) {
				...
				if ( skipped > 0 && imported === 0 ) cls = 'notice notice-error';
				else if ( skipped > 0 )              cls = 'notice notice-warning';
				summaryEl.innerHTML =
					'<div class="' + cls + ' inline" style="padding:10px;margin:0 0 16px"><p style="margin:0">' +
					'<strong>' + imported + '</strong> imported, ' +
					'<strong>' + skipped + '</strong> skipped (of ' + total + ' rows).' +
					'</p></div>';
				...
				// If anything imported, hide the Import button — they're done.
				if ( importBtn ) importBtn.style.display = 'none';
			}

			function runImport() {
				if ( ! fileInput || ! fileInput.files || ! fileInput.files[ 0 ] ) {
					DE.feedback( 'Pick a CSV file first.', 'error' );
					return;
				}
				const activeEv = page.getAttribute( 'data-active-event' ) || '';
				if ( ! activeEv ) {
					DE.feedback( 'No active event.', 'error' );
					return;
				}

				const fd = new FormData();
				fd.append( 'action',   'digitone_events_session_csv_import' );
				fd.append( 'nonce',    DE.nonce );
				fd.append( 'event_id', activeEv );
				fd.append( 'csv',      fileInput.files[ 0 ] );

				importBtn.disabled = true;
				importBtn.textContent = 'Importing…';

				fetch( DE.ajaxUrl, { method: 'POST', credentials: 'same-origin', body: fd } )
					.then( function ( r ) { return r.json(); } )
					.then( function ( res ) {
						importBtn.disabled = false;
						importBtn.textContent = 'Import';
						if ( ! res || ! res.success ) {
							DE.feedback( ( res && res.data && res.data.message ) || 'Import failed.', 'error' );
							return;
						}
						renderResult( res.data );
						if ( ( res.data.imported || 0 ) > 0 ) {
							// Trigger a reload after the user closes the modal so the
							// list reflects the new rows.
							csvModal.addEventListener( 'close', function () {
					...
					.catch( function ( err ) {
						importBtn.disabled = false;
						importBtn.textContent = 'Import';
						DE.feedback( err.message || 'Network error.', 'error' );
					} );
			}
			...
			page.addEventListener( 'click', function ( ev ) {
				...
				if ( ! t ) return;
				const a = t.getAttribute( 'data-de-action' );
				if ( a === 'open-csv-import' )        { ev.preventDefault(); openCsvModal(); }
				if ( a === 'close-csv-modal' )        { ev.preventDefault(); closeCsvModal(); }
				if ( a === 'download-csv-template' )  { ev.preventDefault(); downloadSampleCsv(); }
				if ( a === 'run-csv-import' )         { ev.preventDefault(); runImport(); }
			} );
		}
